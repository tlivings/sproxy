'use strict';

var url = require('url'),
    http = require('http');

/**
 * Creates a new request handler for the given options.
 * @param options
 * @returns {Function}
 * @private
 */
exports = module.exports = function createHandler (options) {
    var _options = {};

    if (typeof options === 'string') {
        _options = url.parse(options);
    }
    else {
        Object.getOwnPropertyNames(options).forEach(function (name) {
            var d = Object.getOwnPropertyDescriptor(options, name);
            Object.defineProperty(_options, name, d);
        });
    }

    /**
     * Pipes the incoming request to the server defined in options.
     * @param req
     * @param res
     * @returns {Function}
     */
    return function (req, res) {
        _fillOptions(_options, req);

        var proxiedReq = http.request(_options, function (proxiedRes) {
            res.writeHead(proxiedRes.statusCode, proxiedRes.headers);
            proxiedRes.pipe(res);
        });

        req.pipe(proxiedReq);
    };
}

/**
 * Fills misc options and headers.
 * TODO: content-length, transfer-encoding, etc.
 * @param options
 * @param req
 * @returns {*}
 * @private
 */
function _fillOptions (options, req) {
    var host = req.headers.host.split(':');

    options.scheme = options.scheme || req.connection.encrypted ? 'https' : 'http';
    options.host = options.host || host[0];
    options.port = options.port || host[1];
    options.path = options.path || req.url;
    options.method = options.method || req.method;

    if (!options.headers) {
        options.headers = {};
    }

    Object.keys(options.headers).forEach(function (header) {
        var name = header.toLowerCase();
        var value = options.headers[header];
        options.headers[name] = value;
        delete options.headers[header];
    });

    options.headers.connection = options.headers.connection || req.headers.connection;

    options.headers['X-Real-IP'] = req.connection.remoteAddress;
    options.headers['X-Forwarded-For'] = options.headers['X-Forwarded-For'] || req.connection.remoteAddress;
    options.headers['X-Forwarded-Proto'] = options.headers['X-Forwarded-Proto'] || options.scheme;

    return options;
}